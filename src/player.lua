class "AI" (Object) {
    ...
    use_big_dude = function(self)
        ...
        for _, com in ipairs(committee_tray.committees) do
            for holder, n in pairs(com.seat_holders) do
                if holder ~= self.agent and holder ~= 'neutral' and n > 0 then
        ...
        for _, type in ipairs(lume.keys(Map.TYPES)) do
            for y = 1, #map.grid do
                for x = 1, #map.grid[1] do
                    ...
                    plan:move_world_coord(mouse_coord * Map.scale)  -- move like a mouse
